...
**Identity models (crypto suites):** identities are pluggable per user via an `alg` tag — `ed25519` (Ed25519 sign + X25519 KEM, the default) or `secp256k1-schnorr` (BIP-340 Schnorr + secp256k1 ECDH, "Nostr"). For `secp256k1-schnorr`, **sign/verify and the KEM are shipped** — a Nostr identity can be a recipient of `delegated`-encrypted collections and a secp256k1 owner can grant access; **still deferred:** multi-device pairing for a secp256k1 root (needs secp256k1 root creation), npub/nsec encoding, and NIP-06 derivation/login. Cap-certs carry `issAlg`/`subAlg`/`subKemAlg` so an `ed25519` root can delegate to (and seal collection keys for) a `secp256k1-schnorr` subject. All `alg` tags are folded into the signed bytes (no downgrade). See [docs/ts/client/26-identity-models.md](docs/ts/client/26-identity-models.md).
