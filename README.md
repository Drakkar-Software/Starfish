...
**Parquet / DuckDB** — `createParquetCollection()` configures a binary collection that accepts Apache Parquet uploads. Files are stored verbatim in the configured object store (S3 in the typical production case); `duckdbReadParquetSql()` generates the DuckDB `httpfs` SQL to query them directly from S3 without a server round-trip. For E2EE parquet datasets (server stores ciphertext; not DuckDB-over-S3 queryable), use `createSealedParquetCollection()` and the `sealAndPushBlob` / `pullAndOpenBlob` helpers from `@drakkar.software/starfish-client`. See [Parquet & DuckDB →](website/docs/analytics/parquet-duckdb.md)
...
| `@drakkar.software/starfish-spaces` | `starfish-spaces` | Multi-user spaces: member roster, shared object tree (two-axis `access × enc`), invite/link join flows, E2EE keyrings, identity links, sealed inbox round-trips, and a server role-enricher |
...
When the server (or QR/relay channel) is not fully trusted, prefer the explicit safety knobs: `assemblePairingBundle({ grantedScope })` bounds a paired device's authority instead of trusting the peer-supplied scope, `installPairingBundle(bundle, device, { expectedRootEdPub, expectedQrNonce })` now **requires root pinning** — pass `expectedRootEdPub` (the account's known root pubkey) or a `confirmUnpinnedRoot` first-contact callback; an unpinned install throws, closing a confused-deputy vuln where an attacker's bundle could bind the device to their own root — and `expectedQrNonce` binds the bundle to its pairing session (the install also fully verifies the cap-cert — signature, expiry window, and `kind === "device"`). `addCollectionRecipient` / `removeRecipient` accept a `trustedAdders` pin so a hostile server cannot substitute a keyring entry. `scopes.admin` is a **device-cap** preset (it manages the keyring); `mintMemberCap` rejects it.
