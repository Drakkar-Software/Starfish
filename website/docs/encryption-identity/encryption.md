...
> **Prerequisites:** [SyncManager](/client-core/sync-manager), [Capability Certificates](/encryption-identity/capability-certs)
...
The full schema, wrap algorithm, lifecycle helpers, and forward-secrecy stance live in [23. Multi-Recipient Delegated Encryption](/encryption-identity/multi-recipient-delegated).

## Wiring a `SyncManager`

The encryptor for a delegated collection is built from the keyring document plus the device's X25519 key pair (recovered at pair time — see [24. Pairing](/encryption-identity/pairing)).
...
For server configuration details see [Config Endpoint](/server/config-endpoint). For the cap-cert auth model that produces those `cap:<op>:<collection>` roles, see [25. Capability Certificates](/encryption-identity/capability-certs).
...
- [Multi-Recipient Delegated Encryption](/encryption-identity/multi-recipient-delegated) — keyring schema, wrap algorithm, epoch rotation, FS stance
- [Capability Certificates](/encryption-identity/capability-certs) — cap-cert minting + the server-side `createCapCertRoleResolver`
- [Pairing](/encryption-identity/pairing) — bootstrap + QR + relay flows that hand the device its key pair and the initial CEK
- [Identity & Key Derivation](/encryption-identity/identity-key-derivation) — how the root identity (and therefore the X25519 KEM key) is derived from the passphrase
