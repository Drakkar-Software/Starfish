...
Cross-language test vectors lock the derivation: [`tests/test-vectors/identity-derivation.json`](https://github.com/Drakkar-Software/Starfish/blob/master/tests/test-vectors/identity-derivation.json). Both the TypeScript and Python implementations must produce byte-identical seeds and `userId` values for every fixture.
...
After bootstrap, plug `creds` into a `StarfishClient` `capProvider` and a `SyncManager` `signer`. See [25. Capability Certificates](/encryption-identity/capability-certs) for the cap-cert schema and [03. SyncManager](/client-core/sync-manager) for the `signer` interface.
...
Locked cross-language by [`tests/test-vectors/identity-derivation-secp256k1.json`](https://github.com/Drakkar-Software/Starfish/blob/master/tests/test-vectors/identity-derivation-secp256k1.json).
...
Locked cross-language by [`tests/test-vectors/identity-derivation-evm.json`](https://github.com/Drakkar-Software/Starfish/blob/master/tests/test-vectors/identity-derivation-evm.json).
...
- Per-device generated keys are what makes [24. Pairing](/encryption-identity/pairing) meaningful: the new device shows its freshly-generated pubkey in the QR / relay request; the root device mints a cap-cert binding that pubkey.
...
- [25. Capability Certificates](/encryption-identity/capability-certs) — minting `device` and `member` caps with these keys
- [24. Pairing](/encryption-identity/pairing) — bootstrap, QR pairing, server-relay invite
- [04. Encryption](/encryption-identity/encryption) — how the X25519 KEM key feeds the per-collection keyring
- [10. Platform Setup](/getting-started/platform-setup) — crypto provider configuration for React Native
