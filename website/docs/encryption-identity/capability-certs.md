...
```json
{
  ...
  "subKem":     "<hex X25519 KEM pub of subject>",
  ...
  "sig":   "<base64 64 bytes = Ed25519 sign(iss, canonical signing input)>"
}
```

Every key is Ed25519 (signing) or X25519 (KEM). `subKem` is the dedicated X25519 KEM key — distinct from `sub` because Ed25519 and X25519 keypairs are separate by design (see [Identity Models](/encryption-identity/identity-models)). `audience` caps carry no subject and so omit `sub`/`subKem`/`subUserId`.
...
1. Well-formedness, **first**, so a malformed `nbf`/`exp` can't slip past the window check: runtime shape validation, then `sha256(iss)[0:32] === issUserId` (and the same for `sub`/`subUserId` when present). The shape check (raises `malformed-shape`):
   ...
   - subject binding is **kind-specific**: an `audience` cap carries **no** `sub`/`subKem`/`subUserId`; a device/member cap requires both `sub` (Ed25519) and `subKem` (X25519);
...
[Root-Only Collections](/server/root-only-collections) for the server-side
...
In-person QR pairing and server-relay invite flows let you add more devices without re-deriving from the passphrase. See [24. Pairing](/encryption-identity/pairing).
