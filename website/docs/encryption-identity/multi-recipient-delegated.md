...
> **Crypto suite:** the keyring KEM is X25519. Every recipient is identified by an X25519 KEM pubkey; collection keys are sealed to it with ephemeral-static X25519 ECDH + HKDF-SHA256. See [Identity Models](/encryption-identity/identity-models) for how external secp256k1 roots bootstrap into a Starfish (Ed25519/X25519) identity.
...
Cross-language test vectors: [`tests/test-vectors/multi-recipient-wrap.json`](https://github.com/Drakkar-Software/Starfish/blob/master/tests/test-vectors/multi-recipient-wrap.json).
...
- [`identity-derivation.json`](https://github.com/Drakkar-Software/Starfish/blob/master/tests/test-vectors/identity-derivation.json) — passphrase → root keypair → userId.
- [`multi-recipient-wrap.json`](https://github.com/Drakkar-Software/Starfish/blob/master/tests/test-vectors/multi-recipient-wrap.json) — full keyring fixture: three recipients, wrap+unwrap roundtrip, addedSig verification.
