...
> **Removed in 3.0:** the built-in `createGroupRoleEnricher` (server-evaluated `groups/{groupId}/members` lists + candidacy). Authorization now flows from signed caps, not server-held membership lists. See the [migration guide](/migration/v2-to-v3#removed-group-role-enricher) for the mapping. `composeEnrichers` and the entitlement enricher are unchanged.

> **Prerequisites:** [Capability Certificates](/encryption-identity/capability-certs), [Config Endpoint](/server/config-endpoint)
...
No enricher is needed; the cap-cert resolver does all the work. The owner revokes by revoking the cap's `nonce` (and, for E2E, rotating the keyring epoch). See [Multi-Recipient Delegated Encryption](/encryption-identity/multi-recipient-delegated).
...
Client flow is unchanged: list days (`GET /list/chats/:groupId`), pull a day, append-and-push a message, poll for updates. For E2E confidentiality the keyring at `chats/{groupId}/_keyring` gates plaintext; the cap gates HTTP access. See [24. Pairing](/encryption-identity/pairing) for how each member's device gets its key pair.
...
- [Capability Certificates](/encryption-identity/capability-certs) — cap-cert schema + the `delegated:<issUserId>:<collection>` role
- [Multi-Recipient Delegated Encryption](/encryption-identity/multi-recipient-delegated) — pair `delegated:` roles with E2E encryption
- [Pairing](/encryption-identity/pairing) — bootstrap, QR, and relay flows
- [Sharing](/sharing/overview) — `mintMemberCap`, scope presets, the `_members` directory
- [Entitlements](/extensions/entitlements) — feature-slug roles composed alongside custom RBAC
- [Identity Action Restrictions](/server/identity-restrictions) — deny access by identity (server / namespace / collection / action)
- [List Endpoint](/server/list-endpoint) — discover documents under a collection prefix
- [Queuing](/extensions/queuing) — react to pushes server-side
