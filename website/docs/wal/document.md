...
`WalDocument` is the client document-log: it turns the [CRDT core](/wal/crdt-model)
...
The high-level [reconcile API](/wal/reconcile), [snapshots](/wal/snapshots), and
the [security model](/wal/security) have their own guides.
...
- **`open()`** is required before mutating. It bootstraps per the reader
  [posture](/wal/snapshots) and resumes the per-author sequence past anything you
  already authored.
...
which the CRDT requires (see [CRDT model](/wal/crdt-model)). The security-related
options are covered in the [security guide](/wal/security).
...
`author` is checked against the signed author — see [security](/wal/security).
...
The reconcile API ([guide 04](/wal/reconcile)) is the recommended way to edit, but
