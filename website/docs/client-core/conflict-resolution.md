...
This requires your domain stores to set `updatedAt: Date.now()` on every item mutation. See [Integration Patterns — Soft Delete](/integration-operations/integration-patterns#soft-delete--tombstone-pattern) for how this interacts with deletions.

### Soft-delete-aware merge

When using [soft deletes](/integration-operations/integration-patterns#soft-delete--tombstone-pattern), the merge must respect tombstones. Without this, a union merge "resurrects" items that one device deleted while another still had them:
...
The key insight: deleted items are **not removed** from the array — they carry a `_deletedAt` timestamp. The merge compares `updatedAt` as usual, so the most recent action (edit or delete) wins per item. See [Soft Delete](/integration-operations/integration-patterns#soft-delete--tombstone-pattern) for the complete pattern.
...
- [Integration Patterns](/integration-operations/integration-patterns) — soft delete, local history, and more
- [Offline & Connectivity](/state-offline/offline-connectivity) — how dirty tracking interacts with conflicts
