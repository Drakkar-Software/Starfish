...
def main() -> None:
    ...
    out = {
        "description": (
            "Cross-language vector for v3.0 capability certificates (ed25519-only "
            "wire). Locks the canonical signing input (stable_stringify of the "
            "cert minus `sig`) and the Ed25519 signature. Verifies `device` "
            "(proxy), `member` (scoped grant), and `audience` (public-link, "
            "no single subject — open and allow-list-restricted) kinds, plus a "
            "forged variant that must fail signature verification."
        ),
        "issuer": {
            "label": "alice_root",
