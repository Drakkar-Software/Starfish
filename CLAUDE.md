...
packages/
  ts/                    # TypeScript packages (pnpm workspaces)
    ...
    wal/                 # @drakkar.software/starfish-wal (extension)
    spaces/              # @drakkar.software/starfish-spaces (extension)
    webhook/             # @drakkar.software/starfish-webhook (extension)
  python/                # Python packages
    ...
    wal/                 # starfish-wal (extension)
    spaces/              # starfish-spaces (extension)
...
- **Versioning**: semver, tag-triggered releases (v*). **All thirty-three packages use lockstep versioning** — every release bumps all packages to the same version number, even if a package has no changes. Packages: `packages/ts/protocol`, `packages/ts/server`, `packages/ts/client`, `packages/ts/keyring`, `packages/ts/identities`, `packages/ts/sharing`, `packages/ts/entitlements`, `packages/ts/restrictions`, `packages/ts/queuing`, `packages/ts/projection`, `packages/ts/audit`, `packages/ts/replica`, `packages/ts/outbox`, `packages/ts/wal`, `packages/ts/spaces`, `packages/ts/webhook`, `packages/ts/events`, `packages/python/protocol`, `packages/python/server`, `packages/python/client`, `packages/python/keyring`, `packages/python/identities`, `packages/python/sharing`, `packages/python/entitlements`, `packages/python/restrictions`, `packages/python/queuing`, `packages/python/projection`, `packages/python/audit`, `packages/python/replica`, `packages/python/outbox`, `packages/python/wal`, `packages/python/spaces`, `packages/python/events`.
...
5. **When adding a new package** — add it to CI in both `.github/workflows/publish-typescript.yml` and/or `.github/workflows/publish-python.yml`:
   - Add test steps in the `test` job (Python only — TS tests run via `pnpm test`)
   - Add a `publish-<name>` job that depends on `setup` + all packages it imports from this repo
   - Set all intra-repo dependency versions in the "Set version from tag" step
   - Also add the package to the lockstep versioning list in the **Versioning** convention above
