...
- **Versioning**: semver, tag-triggered releases (v*). **All thirty-one packages use lockstep versioning** — every release bumps all packages to the same version number, even if a package has no changes. Packages: `packages/ts/protocol`, `packages/ts/server`, `packages/ts/client`, `packages/ts/keyring`, `packages/ts/identities`, `packages/ts/sharing`, `packages/ts/entitlements`, `packages/ts/restrictions`, `packages/ts/queuing`, `packages/ts/projection`, `packages/ts/audit`, `packages/ts/replica`, `packages/ts/outbox`, `packages/ts/wal`, `packages/ts/spaces`, `packages/ts/webhook`, `packages/python/protocol`, `packages/python/server`, `packages/python/client`, `packages/python/keyring`, `packages/python/identities`, `packages/python/sharing`, `packages/python/entitlements`, `packages/python/restrictions`, `packages/python/queuing`, `packages/python/projection`, `packages/python/audit`, `packages/python/replica`, `packages/python/outbox`, `packages/python/wal`, `packages/python/spaces`.
