...
Every key is Ed25519 (signing) or X25519 (KEM). `subKem` is the dedicated X25519 KEM key — distinct from `sub` because Ed25519 and X25519 keypairs are separate by design (see [Identity Models](./26-identity-models.md)). `audience` caps carry no subject and so omit `sub`/`subKem`/`subUserId`.
