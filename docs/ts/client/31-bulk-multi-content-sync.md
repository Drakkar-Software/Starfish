...
One pull, no fan-out. See [Projection](../projection/01-overview.md) for setup.
...
endpoint](../server/list-endpoint.md) to enumerate keys under a prefix, then
...
[one-doc-vs-many decision table](18-multi-document-architecture.md) for when this
makes sense.
...
See [SSE Subscribe Transport](27-sse-subscribe.md) for the full options reference.
...
The [projection extension](../projection/01-overview.md) is also available as
...
- [Multi-Document Architecture](18-multi-document-architecture.md) — when to use one
  document vs many; URL design; partitioning strategies
- [Projection](../projection/01-overview.md) — server-maintained lists; one pull reads
  the whole list
- [List Endpoint](../server/list-endpoint.md) — enumerate keys under a prefix
- [SSE Subscribe Transport](27-sse-subscribe.md) — real-time change notifications
- [KV Pull Cache](29-kv-pull-cache.md) — persist and reuse pull results across
  sessions
