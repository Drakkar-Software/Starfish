...
export async function seal(
  ...
  const ctBuf = await crypto.subtle.encrypt({ name: AES_ALGO, iv: iv as BufferSource }, key, toBytes(plaintext) as BufferSource)
  return { entry, ct: getBase64().encode(concat(iv, new Uint8Array(ctBuf))) }
}
...
export function sealToSelf(
  ...
  return seal(plaintext, selfKemPubHex, sealer)
}

async function open(blob: SealedBlob, recipientKemPrivHex: string): Promise<Uint8Array> {
  ...
  const out = await getCrypto().subtle.decrypt({ name: AES_ALGO, iv: iv as BufferSource }, key, ct as BufferSource)
...
export async function unseal(
  ...
  return open(blob, recipientKemPrivHex)
}
...
export function unsealFromSelf(
  ...
  return unseal(blob, self.kemPrivHex, { requireSealer: self.edPubHex })
}
