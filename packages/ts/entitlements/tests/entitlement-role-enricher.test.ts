...
import { createEntitlementRoleEnricher } from "../src/entitlement-role-enricher.js"
import {
  composeEnrichers,
  createSyncRouter,
  MemoryObjectStore,
...
describe("createEntitlementRoleEnricher — unit", () => {
  ...
  it("treats the exact TTL boundary as expired and re-reads (expiresAt > now is strict)", async () => {
    // The cache hit test is `cached.expiresAt > now`, so at exactly t == expiresAt
    // the entry is NOT served and the store is re-read. Both languages use `>`;
    // pinned to lock the strict boundary. See test_entitlement_role_enricher.py.
    const store = new MemoryObjectStore(new Map())
    await writeEntitlementDoc(store, "users/alice/entitlements", ["premium"])
    ...
    const nowSpy = vi.spyOn(Date, "now")
    nowSpy.mockReturnValueOnce(0) // call 1 → caches with expiresAt = 5000
    await enricher({ identity: "alice", roles: [] }, {})
    expect(getStringSpy).toHaveBeenCalledTimes(1)

    nowSpy.mockReturnValueOnce(5_000) // call 2 at exactly expiresAt → 5000 > 5000 is false → miss
    await enricher({ identity: "alice", roles: [] }, {})
    expect(getStringSpy).toHaveBeenCalledTimes(2)
  })

  it("includes an empty-string feature slug as a bare-prefix role", async () => {
    // An empty string passes the `typeof s === "string"` filter, so it yields a
    // bare-prefix role "entitlement:". Both languages agree (Python's isinstance
    // check also admits ""); pinned as the documented behaviour, not a gap.
    const store = new MemoryObjectStore(new Map())
    await writeEntitlementDoc(store, "users/alice/entitlements", ["", "premium"])
...
describe("composeEnrichers", () => {
  it("merges roles from multiple enrichers", async () => {
    ...
    const teamEnricher = async (auth: AuthResult, params: Record<string, string>): Promise<string[]> =>
      params.groupId === "g1" && auth.identity === "alice" ? ["team-member"] : []
    const entitlementEnricher = createEntitlementRoleEnricher({ store })

    const composed = composeEnrichers(teamEnricher, entitlementEnricher)
...
describe("createEntitlementRoleEnricher — integration via router", () => {
  ...
  it("composeEnrichers: custom + entitlement enrichers both applied", async () => {
    const store = new MemoryObjectStore(new Map())
