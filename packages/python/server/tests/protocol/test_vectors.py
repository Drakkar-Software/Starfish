...
async def test_push_then_pull(scenario):
    ...
    for step in scenario["steps"]:
        if step["action"] == "push":
            ...
            # Regular pull always returns the full document; `step.checkpoint` (if
            # present in the vector) is ignored — checkpoint filtering is now an
            # appendOnly-only concept.
            result = await pull(store, scenario["documentKey"])
