...
# ── push_node_doc: the plaintext-seal guard ──────────────────────────────────
#
# A plaintext node reaching push_node_doc WITH an encryptor is the
# ciphertext-into-objpub failure: the server accepts it (200), the collection is
# declared encryption="none" with read_roles ["public"], and the world reads an
# opaque blob forever. space_access's Tier 0 is the primary defence; this is the
# last check before bytes leave the process, so it holds even if a custom
# SpacePort — or a future refactor that drops the `node` argument again — stops
# feeding the resolver what it needs.


async def test_a_plaintext_node_carrying_an_encryptor_is_refused():
    client = FakeNodeClient(data={"plain": True})
    with pytest.raises(RuntimeError, match="refusing to seal a plaintext node"):
        await default_space_port.push_node_doc(
            Handle(client, FakeEncryptor()),
            "/pull/p",
            "/push/p",
            lambda c: {"x": 1},
            {"access": "public", "enc": False},
        )
    # Nothing may reach the wire — a partial write here is unrecoverable.
    assert client.push_calls == 0


async def test_the_guard_treats_an_absent_enc_key_as_plaintext():
    # create_node stores `enc=True if enc else None`, so a plaintext node carries
    # no `enc` key at all. An `is False` check would miss this shape.
    client = FakeNodeClient(data={"plain": True})
    with pytest.raises(RuntimeError, match="refusing to seal a plaintext node"):
        await default_space_port.push_node_doc(
            Handle(client, FakeEncryptor()),
...
async def test_a_plaintext_node_without_an_encryptor_writes_normally():
    # The guard must not block the legitimate plaintext path it exists to protect.
    client = FakeNodeClient(data={"plain": True})
    await default_space_port.push_node_doc(
        Handle(client, None),
        "/pull/p",
        "/push/p",
        lambda c: {"public": "profile"},
        {"access": "public", "enc": False},
    )
    assert client.data == {"public": "profile"}


async def test_an_encrypted_node_still_seals_with_the_guard_in_place():
    # The control: a guard that refused every encryptor would pass all three
    # tests above. This is what stops that.
    client = FakeNodeClient(missing=True)
    enc = FakeEncryptor()
    await default_space_port.push_node_doc(
        Handle(client, enc),
...
async def test_omitting_node_keeps_the_legacy_permissive_behaviour():
    # `node` stays optional so existing callers keep working; the guard simply
    # cannot fire without it. Pins that it is opt-in, not silently strict.
    client = FakeNodeClient(missing=True)
    await default_space_port.push_node_doc(
        Handle(client, FakeEncryptor()), "/pull/p", "/push/p", lambda c: {"x": 1}
    )
    assert client.data == {"_enc": {"x": 1}}
