...
from starfish_entitlements.entitlement_role_enricher import (
...
@pytest.mark.asyncio
async def test_corrupt_document_result_is_not_cached():
    # A corrupt document yields no entitlements, but that empty result must NOT
    # be cached (matching the TS enricher) — once the document is repaired the
    # entitlements take effect immediately rather than after the cache TTL.
    store = MemoryObjectStore()
    store._data["users/alice/entitlements"] = "not valid json{{"
    enricher = create_entitlement_role_enricher(
        EntitlementRoleEnricherOptions(store=store, cache_ttl_ms=60_000)
    )
    assert await enricher(AuthResult(identity="alice", roles=[]), {}) == []

    _write_entitlement_doc(store, "users/alice/entitlements", ["premium-package-1"])
    roles = await enricher(AuthResult(identity="alice", roles=[]), {})
    assert roles == ["entitlement:premium-package-1"]
...
@pytest.mark.asyncio
async def test_exact_ttl_boundary_is_expired_and_rereads():
    # The cache hit check is `entry.expires_at > now`, so at exactly now ==
    # expires_at the entry is NOT served and the store is re-read. Both languages
    # use `>`; pinned to lock the strict boundary. See entitlement-role-enricher.test.ts.
    store = MemoryObjectStore()
    _write_entitlement_doc(store, "users/alice/entitlements", ["premium"])
    ...
    # t=0 caches with expires_at = 5000ms; t=5.0s == 5000ms hits the boundary exactly.
    with patch("time.monotonic", side_effect=[0.0, 5.0]):
        await enricher(AuthResult(identity="alice", roles=[]), {})
        assert call_count == 1
        await enricher(AuthResult(identity="alice", roles=[]), {})
        assert call_count == 2  # 5000 > 5000 is false → miss → re-read


@pytest.mark.asyncio
async def test_includes_empty_string_feature_slug_as_bare_prefix_role():
    # An empty string passes the isinstance(s, str) filter, so it yields a
    # bare-prefix role "entitlement:". Both languages agree (TS's typeof check also
    # admits ""); pinned as the documented behaviour, not a gap.
    store = MemoryObjectStore()
    _write_entitlement_doc(store, "users/alice/entitlements", ["", "premium"])

    enricher = create_entitlement_role_enricher(EntitlementRoleEnricherOptions(store=store))
    roles = await enricher(AuthResult(identity="alice", roles=[]), {})
    assert sorted(roles) == ["entitlement:", "entitlement:premium"]
...
async def test_compose_merges_roles_from_multiple_enrichers():
    ...
    async def team_enricher(auth, params):
        return ["team-member"] if params.get("groupId") == "g1" and auth.identity == "alice" else []
    entitlement_enricher = create_entitlement_role_enricher(EntitlementRoleEnricherOptions(store=store))

    composed = compose_enrichers(team_enricher, entitlement_enricher)
    roles = await composed(AuthResult(identity="alice", roles=[]), {"groupId": "g1"})

    assert "team-member" in roles
...
@pytest.mark.asyncio
async def test_compose_enrichers_custom_and_entitlement():
    store = MemoryObjectStore()
    ...
    col = CollectionConfig(
        ...
        readRoles=["team-member", "entitlement:premium-package-1"],
        writeRoles=["admin"],
        encryption="none",
        maxBodyBytes=65536,
    )
